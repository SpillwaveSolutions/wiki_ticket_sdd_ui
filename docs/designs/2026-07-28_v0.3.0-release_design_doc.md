---
wiki_key: design/2026-07-28_v0.3.0-release-design-doc
doc_type: design
...
`v0.3.0`: `server/` (Hono JSON API + static host), `web/` (Vite/React/Tailwind
frontend, ten panels), `src-tauri/` (Tauri 2 desktop shell, a full Rust port
of the same 13 endpoints, plus a Tauri-only repo-discovery subsystem —
`appconfig.rs`, 11 new commands, `GithubRepoPanel.tsx`/`LocalRootsPanel.tsx`).
Also in scope: `bin/worklog` as the external contract this app shells out to,
and the `docs/.index/` plane it reads.
...
- `docs/plans/2026-07-27-repo-picker-discovery.md` — the repo-picker discovery
  plan (local-root scanning, GitHub org search, shallow-clone cache; source
  of §6 decision 6, §8.4, and the new rows in §7/§11/§14)
...
**New in v0.3.0 — repo-picker discovery (desktop-only).** Picking a target
repo no longer requires already knowing its path: the desktop shell's "Repo"
modal gained a **Local** tab that scans configured root directories (e.g.
`~/src`) one level deep for worklog-enabled checkouts, and a **GitHub** tab
that lists the operator's accessible orgs/repos via `gh`, progressively
filters them down to worklog-enabled ones, and can shallow-clone (`--depth
1`) a not-yet-local repo into a managed per-machine cache before opening it
(`src-tauri/src/appconfig.rs`, new; 11 new Tauri commands in
`src-tauri/src/commands.rs`; `web/src/components/LocalRootsPanel.tsx` and
`GithubRepoPanel.tsx`, new). This is additive and Tauri-only — browser mode
is unchanged. See §6 decision 6, §8.4, §14.
...
- (New in v0.3.0, desktop-only) Discover candidate repos instead of
  requiring the operator already know a path: scan configured local root
  directories, or search/clone from GitHub via `gh` (§8.4).
...
| Path-traversal safety | `resolveDocPath` (`server/src/repo.ts:108-115`), `resolve_doc_path` (`src-tauri/src/repo.rs:146-170`) |
...
- **`src-tauri/`** — Tauri 2 Rust shell. Same 13 operations as
  ...
  no meaningful cwd and no `--repo` flag to relaunch with. Adds 11 more
  desktop-only commands for repo *discovery* (§6 decision 6, §8.4): local
  root-directory scanning, GitHub org/repo listing and shallow-clone via
  `gh`, and a managed clone cache — backed by the new
  `src-tauri/src/appconfig.rs` module, which owns a separate per-machine app
  config (`~/.config/wicked_ticket/config.json`) distinct from any target
  repo's own `.work/config.yml`.
...
**Decision 6 — Repo discovery is Tauri-only, backed by a separate app-level
config file, not the target repo's own config.** *Context:*
`docs/plans/2026-07-27-repo-picker-discovery.md`. Before v0.3.0, choosing a
target repo required already knowing its filesystem path (native folder
dialog) or having launched it before (remembered `localStorage` history) —
there was no way to *discover* candidate repos. *Alternatives considered:*
(a) extend browser mode too, with a server-side filesystem scan endpoint;
(b) store discovered roots inside the target repo's own `.work/config.yml`.
*Selected:* Tauri-only (native `fs`/`gh` shell-out has no browser
equivalent, matching the existing `pick_repo`/`set_repo` scope boundary),
with roots and cache location persisted in a new, separate app-level file,
`~/.config/wicked_ticket/config.json` (`src-tauri/src/appconfig.rs:20-27`).
*Rationale:* (a) a browser-mode filesystem-scan endpoint would need to trust
arbitrary paths from an HTTP request — a materially different security
posture than a local desktop dialog; (b) app-level preferences (which root
directories the operator scans) are not part of any one target repo's
identity and must survive switching between target repos, so they cannot
live in a target's own `.work/config.yml` without also violating the
read-only guarantee (CLAUDE.md rule 3) the moment the app ever wrote to it.
*Tradeoff:* browser-mode users get no discovery UI at all — accepted, since
browser mode already has an equivalent "remembered recent paths" UX via
`localStorage`. *Consequence:* the shallow-clone cache
(`src-tauri/src/appconfig.rs:29-31, 110-140`) is the one place this codebase
writes to disk *outside* a target repo — never inside one, so the
Component 3 (read-only) guarantee is unaffected; `clean_cache`
(`src-tauri/src/appconfig.rs:144-168`) guards against deleting anything
outside its own managed `cache_root()` (verified by
`clean_cache_rejects_paths_outside_cache_root`, `appconfig.rs:244-254`).
*Revisit if:* browser mode ever needs the same discovery UX.
...
| GitHub REST/`gh` | External service | Release list; (new) org/repo listing + shallow clone for discovery | `ticketing.project` from config; org name / owner+name for discovery | JSON release array; `GhRepoCandidate[]`; cloned repo on disk | Network / `gh` auth | Falls back through 3 tiers to `{offline: true, releases: []}` (releases); discovery commands return a 400 if `gh` is missing/unauthenticated |
| `src-tauri/src/appconfig.rs` (new, desktop-only) | Rust module | App-level config CRUD (`repo_roots`), local-root scanning, shallow-clone cache directory + cleanup | Root paths, `HOME` env | `AppConfig`, `LocalRepoCandidate[]`, `CachedRepoInfo[]` | Filesystem only (`~/.config/wicked_ticket/`) | A missing/corrupt config file degrades to `AppConfig::default()` (empty roots), never a crash |
| `web/src/components/LocalRootsPanel.tsx` (new) | Frontend (desktop-only) | Add/remove scan roots, list worklog-enabled repos found under them | `api.listRepoRoots`/`scanLocalRepos` | Selected repo path → `onSelect` | `lib/api.ts` discovery wrappers | Scan failure shows an empty list, not an error banner |
| `web/src/components/GithubRepoPanel.tsx` (new) | Frontend (desktop-only) | Org picker, progressively-filtered worklog-enabled repo list, clone action, cache management | `api.listGhOrgs`/`listOrgRepos`/`checkWorklogEnabled` | Cloned `RepoInfo` → `onCloned` | `lib/api.ts` discovery wrappers | `gh` unavailable/unauthenticated shows an inline error with a remediation hint, not a crash |
...
env, or cwd as last resort — `server/src/repo.ts:82-89`). A relative
`--repo` value is resolved against `INIT_CWD` (npm's own record of the
directory `npm start` was actually invoked from) when set, falling back to
`process.cwd()` only for a direct `node dist/index.js` invocation — **fixed
in v0.3.0** (commit `6fb7f3d`): npm workspace scripts (`npm run -w server
start --`) run with `process.cwd()` set to `server/`, not the invoking
directory, so a bare `path.resolve(candidate)` previously resolved
`--repo ../wiki_ticket_sdd` one level too shallow. Regression-tested by
three new cases in `server/test/repo.test.ts` (relative/absolute/no-
`INIT_CWD`). The server binds
...
### 8.4 Repo discovery: local scan + GitHub org search + shallow clone (new, desktop-only)

Trigger: opening the "Repo" modal's **Local** or **GitHub** tab
(`web/src/components/RepoPickerModal.tsx:127-142, 174-175`). Preconditions:
none for **Local** (an empty `repo_roots` list is a valid first-run state);
**GitHub** requires `gh` installed and authenticated, else the panel renders
an inline remediation message instead of an error
(`web/src/components/GithubRepoPanel.tsx:108-119`). Main flow — **Local**:
`LocalRootsPanel` calls `listRepoRoots` + `scanLocalRepos` in parallel
(`web/src/components/LocalRootsPanel.tsx:22-34`); `scan_local_repos`
(`src-tauri/src/commands.rs:616-618` → `appconfig::scan_local_repos`,
`src-tauri/src/appconfig.rs:76-101`) walks each configured root **one level
deep**, keeping only directories containing `.git`, and flags
`worklog_enabled` by checking for `.work/config.yml`; the frontend filters
to `worklog_enabled` results before rendering (`LocalRootsPanel.tsx:28`).
Main flow — **GitHub**: `GithubRepoPanel` calls `list_gh_orgs`
(`src-tauri/src/commands.rs:621-660`, `gh api user` for the operator's own
login + `gh api user/orgs`) to populate an org dropdown, then `list_org_repos`
(`commands.rs:661-710`, `gh repo list <org> --limit 200 --json ...`) to list
that org's repos (capped at 200, no pagination — explicit scope cut), then
progressively calls `check_worklog_enabled` per repo
(`commands.rs:711-724`, `gh api repos/{owner}/{name}/contents/.work/config.yml`)
with capped concurrency (`WORKLOG_CHECK_CONCURRENCY = 6`,
`GithubRepoPanel.tsx:9, 12-21, 61-66`) so the UI reveals worklog-enabled
repos as each check resolves rather than blocking on the whole org. Clicking
"Clone & open" calls `clone_repo` (`commands.rs:726-758`): shallow-clones
(`gh repo clone {owner}/{name} {dest} -- --depth 1`) into
`~/.config/wicked_ticket/clones/{owner}/{name}` if not already cached, then
validates and stores the path via the same `apply_repo_path` every other
repo-selection path uses (§8.1), so a bad clone can never be silently
trusted. Cache entries are listed (`list_cached_repos`) and removable
individually or all-at-once (`clean_repo_cache`,
`GithubRepoPanel.tsx:169-199`).

```mermaid
sequenceDiagram
    participant U as User
    participant Local as LocalRootsPanel
    participant GH as GithubRepoPanel
    participant Cmd as commands.rs
    participant Cfg as appconfig.rs
    participant GhCli as gh CLI

    U->>Local: open "Local" tab
    Local->>Cmd: scan_local_repos()
    Cmd->>Cfg: scan_local_repos()
    Cfg-->>Cmd: LocalRepoCandidate[] (one level deep)
    Cmd-->>Local: worklog-enabled subset
    U->>Local: click a repo
    Local->>Cmd: set_repo(path) [existing §8.1 flow]

    U->>GH: open "GitHub" tab
    GH->>Cmd: list_gh_orgs()
    Cmd->>GhCli: gh api user / user/orgs
    GhCli-->>Cmd: logins
    Cmd-->>GH: orgs[]
    U->>GH: pick org
    GH->>Cmd: list_org_repos(org)
    Cmd->>GhCli: gh repo list <org>
    GhCli-->>Cmd: repos[]
    loop per repo, concurrency <= 6
        GH->>Cmd: check_worklog_enabled(owner, name)
        Cmd->>GhCli: gh api .../contents/.work/config.yml
    end
    U->>GH: "Clone & open"
    GH->>Cmd: clone_repo(owner, name)
    Cmd->>GhCli: gh repo clone --depth 1
    Cmd->>Cmd: apply_repo_path() [validate then store]
    Cmd-->>GH: RepoInfo
    GH->>U: reload against cloned repo
```

Failure flow: `gh` missing/unauthenticated fails `list_gh_orgs` with a 400
the panel renders as a remediation hint (`gh auth login`), not a crash; a
failed clone leaves no partial state trusted (`apply_repo_path` only runs
after `gh repo clone` exits 0). Read-only guarantee: unaffected — the only
filesystem writes here are into the app's own managed cache directory, never
into a target repo (§6 decision 6).
...
**Rule** (`server/src/repo.ts:108-115`, `src-tauri/src/repo.rs:146-170`):
...
`require_repo()` (lines 61-69) and delegate. Two additional commands with no
Node equivalent: `pick_repo`/`set_repo` (lines 551-579), both routing through
`apply_repo_path()` (lines 524-535) which validates-then-stores — the same
"validate before trusting" pattern as the Node middleware, just triggered at
selection time instead of per-request. **New in v0.3.0** (§6 decision 6): an
11-command repo-discovery block (`commands.rs:581-768`) — `list_repo_roots`,
`add_repo_root`, `remove_repo_root`, `pick_repo_root`, `scan_local_repos`
delegate straight to `appconfig::*`; `list_gh_orgs`, `list_org_repos`,
`check_worklog_enabled` shell out to `gh api`/`gh repo list`; `clone_repo`
shells out to `gh repo clone ... --depth 1` into the managed cache then
reuses `apply_repo_path()`; `list_cached_repos`/`clean_repo_cache` also
delegate to `appconfig::*`. None of these have an HTTP/`routes.ts`
counterpart — see §14.

**`src-tauri/src/appconfig.rs`** (new, 269 lines) — app-level config, wholly
separate from any target repo's `.work/config.yml`: `AppConfig { repo_roots:
Vec<String> }` persisted at `config_path()` = `~/.config/wicked_ticket/config.json`
(lines 20-27); `load`/`save` (lines 33-50) degrade to `AppConfig::default()`
on any read error rather than erroring; `add_root`/`remove_root` (lines
52-66) dedup on insert. `scan_local_repos` (lines 76-101) walks each
configured root **one level deep only** (an explicit scope cut — not
recursive), keeping child directories that contain `.git`, and flags
`worklog_enabled` via a `.work/config.yml` existence check. `cache_root()`
= `~/.config/wicked_ticket/clones` (line 30); `list_cached_repos` (lines
111-140) walks it two levels deep (`<owner>/<name>`). `clean_cache` (lines
144-168) is the one security-relevant control in this module: it refuses to
`remove_dir_all` any path that does not start with `cache_root()`, so it can
never be used to delete an arbitrary `repo_roots` entry or any other
path — verified by `clean_cache_rejects_paths_outside_cache_root`
(lines 245-254). 5 `#[cfg(test)]` unit tests (lines 198-268) use a
temp-`HOME` guard (`with_temp_home`, lines 183-196, serialized via a
`Mutex` since `HOME` is process-global) to cover: empty-config load,
add/remove/dedup roundtrip, local-repo scan filtering, and both cache-guard
cases.
...
(debug-only) log plugins, manages `AppState`, lists all 27 commands
(13 read + `pick_repo` + `set_repo` + 11 new repo-discovery commands) in
`invoke_handler!` (lines 33-61); `pub mod appconfig;` added at line 1.
...
**Repo-discovery components** (`web/src/components/*.tsx`, new in v0.3.0,
Tauri-only — see §8.4): **`LocalRootsPanel.tsx`** (120 lines) manages the
root-directory list (add via native dialog, remove) and renders the
worklog-enabled subset of `scanLocalRepos()`'s results
(`LocalRootsPanel.tsx:22-34`); **`GithubRepoPanel.tsx`** (202 lines) owns
org selection, the progressive worklog-enabled filter
(`mapWithConcurrency`, lines 12-21, capped at `WORKLOG_CHECK_CONCURRENCY =
6`), clone/clean actions, and cached-clone listing. **`RepoPickerModal.tsx`**
(reworked, +84/-29 lines) hosts both as a small Recent/Local/GitHub tab bar
(lines 127-142) gated on `isTauri()` — browser mode keeps its original
Recent-only layout unchanged (lines 144-172).
...
**Repo-discovery commands (new in v0.3.0, all Tauri-only — no HTTP route
exists or is planned; §6 decision 6, §8.4):**

| Tauri command | Purpose | Notable errors |
|---|---|---|
| `list_repo_roots` | Read configured scan roots from app config | Never errors — empty list if unconfigured |
| `add_repo_root{path}` | Add a root, deduped | — |
| `remove_repo_root{path}` | Remove a root | — |
| `pick_repo_root` | Native folder dialog → add as a root | 400 if cancelled |
| `scan_local_repos` | One-level-deep scan of all roots for `.git` dirs, flagged `worklog_enabled` | Never errors — skips unreadable roots |
| `list_gh_orgs` | Operator's own login + `gh api user/orgs` | 400 if `gh` missing/unauthenticated |
| `list_org_repos{org}` | `gh repo list <org> --limit 200 --json ...` | 400 if `gh repo list` fails |
| `check_worklog_enabled{owner,name}` | `gh api repos/{owner}/{name}/contents/.work/config.yml` | Returns `false` on any failure, never errors |
| `clone_repo{owner,name}` | Shallow clone (`--depth 1`) into the managed cache if not already present, then `apply_repo_path()` | 500 if `gh repo clone` fails/exits nonzero |
| `list_cached_repos` | List `<owner>/<name>` dirs under the managed cache | Never errors — empty list if cache absent |
| `clean_repo_cache{path?}` | Remove one cached clone, or the whole cache if `path` omitted | 400 if `path` is outside the managed cache root |
...
**GitHub (new in v0.3.0 — repo discovery, desktop-only, `gh`-only, no REST
fallback).** Unlike releases, the discovery commands (§8.4, §14) have **no
unauthenticated-REST or offline tier** — they require `gh` installed and
authenticated, and fail with a clear 400 if not (`list_gh_orgs`,
`src-tauri/src/commands.rs:621-660`). *Rationale (assumption, not stated
explicitly in the plan):* org/repo listing and cloning are
authorization-scoped operations (the operator's own accessible orgs/repos),
unlike the public release-list read, so an unauthenticated fallback would
either expose nothing useful or require re-implementing GitHub's OAuth flow
— out of scope for a local tool that already depends on `gh` elsewhere.
Calls used: `gh api user`, `gh api user/orgs`, `gh repo list <org> --limit
200 --json name,owner,description,isPrivate`, `gh api
repos/{owner}/{name}/contents/.work/config.yml` (existence check, one call
per candidate repo, capped at 6 concurrent), `gh repo clone {owner}/{name}
{dest} -- --depth 1`. All read-only except the clone, which writes only to
the app's own managed cache directory, never to an existing target repo
(§6 decision 6).
...
**Cache-deletion guard (new in v0.3.0).** `clean_repo_cache`/`clean_cache`
(`src-tauri/src/appconfig.rs:144-168`) is a second attacker-shaped-input
control alongside the doc-path guard: its optional `path` argument comes
from the frontend, and the handler refuses to `remove_dir_all` anything
that does not start with `cache_root()` (`~/.config/wicked_ticket/clones`),
so it cannot be used to delete an arbitrary `repo_roots` entry or any other
filesystem path. Tested by
`clean_cache_rejects_paths_outside_cache_root` (`appconfig.rs:245-254`).
...
| Arbitrary path deletion via `clean_repo_cache` | `appconfig::clean_cache` (new, v0.3.0) | Low (local tool, attacker-shaped `path` arg) | Deletion of an arbitrary directory on the operator's machine | `starts_with(cache_root())` guard, unit-tested |
...
`WORKLOG_REPO` env > cwd (`server/src/repo.ts:82-89`) — a relative `--repo`
resolves against `INIT_CWD` when set (npm's record of the real invocation
directory), falling back to `process.cwd()` otherwise; fixed in v0.3.0,
commit `6fb7f3d` (see §8.1); `--port` flag >
...
**App-level configuration (new in v0.3.0, desktop-only, §6 decision 6):**
`~/.config/wicked_ticket/config.json` (`src-tauri/src/appconfig.rs:20-27`)
holds `repo_roots` — the operator's own configured local-scan directories —
plus implicitly defines the managed shallow-clone cache location
(`~/.config/wicked_ticket/clones/`, `appconfig.rs:29-31`). This is
per-machine app state, not part of any target repo, and is the only file
this app writes to outside a target repo's own tree. **No secrets** are
read, stored, or required by this app itself.
...
exercises the real `../wiki_ticket_sdd` repo. **`appconfig.rs`
(`#[cfg(test)]`, new in v0.3.0):** 5 tempdir-based unit tests
(`src-tauri/src/appconfig.rs:198-268`) mirroring `repo.rs`'s existing style,
using a `with_temp_home` guard serialized on a `Mutex` (since `HOME` is
process-global) — cover empty-config load, add/remove/dedup roundtrip,
one-level-deep local-repo scan filtering (`.git`/`.work/config.yml`), and
both `clean_cache` guard cases (rejects outside `cache_root()`, removes one
entry vs. all). No parity-suite equivalent needed — this module has no Node
counterpart (Tauri-only, §6 decision 6). **Critical edge cases already
...
comment, `gh`-missing → REST fallback → offline chain (§20), relative
`--repo` resolution against `INIT_CWD` vs. `process.cwd()` (§8.1, §26),
cache-deletion guard rejecting paths outside `cache_root()` (§22).
...
| Repo-picker discovery (`docs/plans/2026-07-27-repo-picker-discovery.md`) | §8.4 | `src-tauri/src/appconfig.rs`, `commands.rs`, `LocalRootsPanel.tsx`, `GithubRepoPanel.tsx` | `scan_local_repos`, `list_org_repos`, `clone_repo`, `clean_cache` | `appconfig.rs` `#[cfg(test)]` (5 tests); no Node counterpart (Tauri-only) |
...
flowchart; §8.1 repo-selection sequence diagram; §8.4 repo-discovery
sequence diagram (local scan + GitHub org search + clone, new in v0.3.0);
§9.1 drift state diagram; §10 domain class diagram; §11 module dependency
flowchart; §14 wiki-ledger sequence diagram; §20 releases fallback sequence
diagram; §23 error-handling activity diagram; §27 deployment flowchart.
